...
(async () => {
  ...
  try {
    ...
    await page.fill('input[placeholder="Alex Sterling"]', 'Audit Tester');
    await page.fill('input[type="email"]', signupEmail);
    await page.fill('input[placeholder="Create strong master key"]', 'SuperSecurePass123!');
    ...
    // Submit signup
    await page.click('button:has-text("Deploy Secure Account")');
    ...
    await page.click('text=Access Core');
    await page.waitForTimeout(1000);

    await page.fill('input[type="email"]', testEmail);
    await page.fill('input[placeholder="Enter secure vault password"]', testPassword);
    await page.click('button:has-text("Unlock Secure Layer")');
    ...
    if (await logoutBtn.count() > 0) {
      ...
      console.log('Warning: Logout button not found in current view.');
    }
    await page.waitForTimeout(5000);
